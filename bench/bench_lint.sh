#!/usr/bin/env bash
# Lint benchmark: sanz vs oxlint vs biome — default rulesets, matched thread counts.
#
#   sanz:   292 ESLint core rules (--eslint-plugin eslint)
#   oxlint:  93 default rules
#   biome:  ~150 default rules
#
...
run() {
  ...
  hyperfine --warmup 1 --runs "$runs" --ignore-failure \
    ...
    --command-name "biome (~150 rules)"    "biome lint --vcs-use-ignore-file=false $corpus"
  echo ""
}
