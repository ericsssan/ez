...
fn benchFile(gpa: std.mem.Allocator, io: std.Io, path: []const u8) !void {
    ...
    for (0..N) |_| {
        // ── Sequential: lex then parse ──
        {
            ...
            const tot_ns: u64 = lex_ns + parse_ns;
    ...
    const overhead = con_us - max_us;
    std.debug.print("  {s:<40} {d:>9} B  | seq {d:>6}us (lex {d:>5} + parse {d:>5}) | conc {d:>6}us | saved {d:>5}us ({d:>3}%) | ovhd {d:>4}us\n", .{
        path, source.len, seq_us, lex_us, parse_us, con_us, saved, pct, overhead,
    });
}
