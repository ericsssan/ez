...
test "parseConfigJson with categories" {
    ...
    // Style rules should be off. no-var is the first style rule (index 68).
    // Count: 40 correctness + 28 suspicious = 68
    try std.testing.expect(config.rule_severity_table[68] == .off);
}
