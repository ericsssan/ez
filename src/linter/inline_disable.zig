...
pub const DisableDirective = struct {
    ...
    pub const Kind = enum {
        disable, // ez-disable — disable from this line onward
        disable_next_line, // ez-disable-next-line — disable for next line only
        enable, // ez-enable — re-enable from this line onward
    };
};
...
pub const InlineDisables = struct {
    ...
    fn tryParseDirective(comment_body: []const u8, line: u32) ?DisableDirective {
        ...
        const next_line_prefix = "ez-disable-next-line";
        const disable_prefix = "ez-disable";
        const enable_prefix = "ez-enable";
        ...
        if (rest.len > 0 and rest[0] != ' ' and rest[0] != '\t' and rest[0] != '*' and rest[0] != '\n') {
            return null; // e.g. "ez-disablefoo" is not a valid directive
        }
...
test "parse disable-next-line" {
    const source = "const x = 1;\n// ez-disable-next-line no-unused-vars\nconst y = 2;\n";
...
test "parse disable/enable range" {
    const source = "// ez-disable no-console\nconsole.log('a');\nconsole.log('b');\n// ez-enable no-console\nconsole.log('c');\n";
    var disables = try InlineDisables.parse(std.testing.allocator, source);
    defer disables.deinit();
...
test "isSuppressed next line" {
    const source = "// ez-disable-next-line no-unused-vars\nconst x = 1;\nconst y = 2;\n";
    var disables = try InlineDisables.parse(std.testing.allocator, source);
    defer disables.deinit();
...
test "not suppressed in string" {
    const source = "const x = \"// ez-disable no-console\";\nconsole.log('hi');\n";
    var disables = try InlineDisables.parse(std.testing.allocator, source);
    defer disables.deinit();
