...
fn literalValue(init: NodeIndex, ctx: *const LintContext) Value {
    ...
    if (tag == .number_literal) {
        return canonicalNumber(ctx.tokenText(ctx.nodeMainToken(n)), false);
    }
    if (tag == .unary_minus or tag == .unary_plus) {
        const inner = ctx.nodeData(n).lhs;
        if (inner == .none or ctx.nodeTag(inner) != .number_literal) return .{};
        return canonicalNumber(ctx.tokenText(ctx.nodeMainToken(inner)), tag == .unary_minus);
    }
