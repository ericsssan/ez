...
// TS variant of no-array-constructor with messageId "useLiteral" instead of "preferLiteral".
...
pub fn runOnSymbols(ctx: *const LintContext) void {
    ...
    while (r < count) : (r += 1) {
        ...
        {
            ctx.reportWithMessageId(parent, "useLiteral");
            ...
            ctx.reportWithFixAndMessageId(parent, .{ .start = span.start, .end = span.end }, fix_text, "useLiteral");
        } else {
            const fix_text = std.fmt.allocPrint(ctx.allocator, "[{s}]", .{ctx.argsTextBetweenParens(parent)}) catch return;
            defer ctx.allocator.free(fix_text);
            ctx.reportWithFixAndMessageId(parent, .{ .start = span.start, .end = span.end }, fix_text, "useLiteral");
