...
fn checkOptionalChainReceiver(recv: NodeIndex, op_node: NodeIndex, ctx: *const LintContext) void {
    ...
    var n = recv;
    while (ctx.nodeTag(n) == .grouping_expr) n = ctx.nodeData(n).lhs;
    // Bail whenever the chain contains an array indexed access — under
    // noUncheckedIndexedAccess (which we can't read from tsconfig) the
    // index could be undefined and "infects" every subsequent `?.` link.
    if (containsArrayIndexedAccess(n, ctx)) return;
