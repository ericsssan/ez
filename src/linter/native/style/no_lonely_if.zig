...
pub fn run(node: NodeIndex, ctx: *const LintContext) void {
    if (!(((blk: { const __t = ctx.nodeTag(node); break :blk (__t == .if_stmt or __t == .if_else_stmt); } and !((conditionalChild(ctx, node, .alternate) != .none))) and ((((((ctx.nodeTag(ctx.parentOf(node)) == .block_stmt) and (ctx.nodeBodyStmtCount(ctx.parentOf(node)) == 1)) and (ctx.nodeBodyStmtAt(ctx.parentOf(node), 0) == node)) and (blk: { const __t = ctx.nodeTag(ctx.parentOf(ctx.parentOf(node))); break :blk (__t == .if_stmt or __t == .if_else_stmt); } and !((conditionalChild(ctx, ctx.parentOf(ctx.parentOf(node)), .alternate) != .none)))) and (conditionalChild(ctx, ctx.parentOf(ctx.parentOf(node)), .consequent) == ctx.parentOf(node))) or ((blk: { const __t = ctx.nodeTag(ctx.parentOf(node)); break :blk (__t == .if_stmt or __t == .if_else_stmt); } and !((conditionalChild(ctx, ctx.parentOf(node), .alternate) != .none))) and (conditionalChild(ctx, ctx.parentOf(node), .consequent) == node)))))) {
        return;
    }
