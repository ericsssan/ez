...
const TokenTag = @import("es_parser").token.Tag;
...
pub fn run(node: NodeIndex, ctx: *const LintContext) void {
    ...
    switch (tag) {
        ...
        .constructor_def => {
            ...
            if (is_private and allowHas(ctx, "privateConstructors", "private-constructors")) return;
            if (is_protected and allowHas(ctx, "protectedConstructors", "protected-constructors")) return;
            reportEmpty(node, body, ctx);
            return;
        },
        .method_def, .computed_method_def => {
            ...
            if (tag == .method_def) {
                ...
                if (std.mem.eql(u8, ctx.tokenText(key_tok), "constructor")) {
                    ...
                    if (acc == ModifierBit.acc_private and allowHas(ctx, "privateConstructors", "private-constructors")) return;
                    if (acc == ModifierBit.acc_protected and allowHas(ctx, "protectedConstructors", "protected-constructors")) return;
                    reportEmpty(node, body, ctx);
                    return;
                }
            }
...
/// ESLint reports `node.body.loc` (the `{}` block span) with messageId
/// "unexpected".  Use reportSpanWithMessageId on the body span.
fn reportEmpty(node: NodeIndex, body: NodeIndex, ctx: *const LintContext) void {
    _ = node;
    if (body == .none) return;
    const sp = ctx.nodeSpan(body);
    ctx.reportSpanWithMessageId(sp, "unexpected");
}

/// Check `allow` for either of two aliases.  TSe's
/// `@typescript-eslint/no-empty-function` uses kebab-case for the
/// constructor-accessibility keys (`private-constructors`,
/// `protected-constructors`) while ESLint core uses camelCase
/// (`privateConstructors`, `protectedConstructors`).
fn allowHas(ctx: *const LintContext, a: []const u8, b: []const u8) bool {
    if (ctx.optionArrayContains("allow", a)) return true;
    if (ctx.optionArrayContains("allow", b)) return true;
    return false;
}
