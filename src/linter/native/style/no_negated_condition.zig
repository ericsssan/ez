...
pub fn run(node: NodeIndex, ctx: *const LintContext) void {
    switch (ctx.nodeTag(node)) {
        .if_stmt, .if_else_stmt => {
            if (!(((conditionalChild(ctx, node, .alternate) != .none) and !(blk: { const __t = ctx.nodeTag(conditionalChild(ctx, node, .alternate)); break :blk (__t == .if_stmt or __t == .if_else_stmt); })))) {
                return;
            }
