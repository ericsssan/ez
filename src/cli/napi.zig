...
fn parseImpl(
    ...
    if (semantic_mod.SemanticAnalyzer.analyze(sem_arena.allocator(), &tree)) |sem_result| {
        ...
        if (js_buffer.writeSemanticData(buf_ptr, &backing, &sem, @intCast(tree.nodes.len), tree.nodes.items(.tag), traversal.pre_order, traversal.parents)) |off| {
...
fn parseAndLintImpl(
    ...
    if (semantic_mod.SemanticAnalyzer.analyze(sem_arena.allocator(), &tree)) |sr| {
        sem_result_opt = sr;
        if (js_buffer.writeSemanticData(buf_ptr, &backing, &sem_result_opt.?, @intCast(tree.nodes.len), tree.nodes.items(.tag), traversal.pre_order, traversal.parents)) |off| {
            semantic_data_offset = off;
        } else |_| {}
    } else |_| {}
