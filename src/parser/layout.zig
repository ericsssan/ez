...
pub export fn sx3_tag_count() u32 {
    return tag_count;
}

/// Returns the ESTree-compatible type name for a given tag index.
pub export fn sx3_tag_name(index: u8) [*:0]const u8 {
...
test "sx3_tag_count matches enum" {
    try std.testing.expectEqual(tag_count, @as(u32, @typeInfo(NodeTag).@"enum".fields.len));
}

test "sx3_tag_name out of bounds" {
    try std.testing.expectEqualStrings("Unknown", std.mem.span(sx3_tag_name(255)));
}
