...
function validateRule(rule) {
  ...
  for (let hi = 0; hi < rule.handlers.length; hi++) {
    ...
    if (h.kind === "for-each-ref-by-name") {
      if (typeof h.messageId !== "string") return fail("messageId must be string", path);
    ...
    if (h.kind === "no-dupe-args-check") {
      if (typeof h.messageId !== "string") return fail("messageId must be string", path);
      continue;
    }
    if (h.kind === "no-dupe-keys-check") {
      if (typeof h.messageId !== "string") return fail("messageId must be string", path);
      continue;
    }
    if (h.kind === "no-dupe-class-members-check") {
      if (typeof h.messageId !== "string") return fail("messageId must be string", path);
      continue;
    }
    if (h.kind === "no-unused-labels-check") {
      if (typeof h.messageId !== "string") return fail("messageId must be string", path);
      continue;
    }
    ...
    if (h.kind === "no-sparse-arrays-check") {
      if (typeof h.messageId !== "string") return fail("messageId must be string", path);
      continue;
    }
    ...
    if (h.kind === "no-invalid-regexp-check") continue;
    if (h.kind === "no-misleading-char-class-check") continue;
    if (h.kind === "no-misleading-char-class-call-check") continue;
